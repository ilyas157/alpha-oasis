...
const VoyageFormComplete = () => {
  ...
  const getVoyageStats = () => {
    const etapes = voyageForm.etapes || [];

    const totalEtapes = etapes.length;

    const totalTrajets = etapes.reduce(
      (acc, etape) => acc + (etape.trajets?.length || 0),
      0
    );

    const totalSejours = etapes.reduce(
      (acc, etape) => acc + (etape.séjour?.length || 0),
      0
    );

    const totalJours = etapes.reduce((acc, etape) => {
      const sejours = etape.séjour || [];
      return (
        acc +
        sejours.reduce(
          (sejourAcc, sejour) => sejourAcc + (sejour.jours?.length || 0),
          0
        )
      );
    }, 0);

    return { totalEtapes, totalTrajets, totalSejours, totalJours };
  };

  // Preview card helpers
  const stats = getVoyageStats();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Voyage créer avec succès:', response.data);
      alert('Voyage créer avec succès !');
      //setVoyageForm(response.data); // ✅ Optionnel: maj l'état local
    } catch (error) {
      console.error('Erreur création voyage:', error.response.data.error);
      alert(
        error.response?.data?.error || 'Erreur lors de la création du voyage.'
      );
    } finally {
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      type="date"
                      value={
                        voyageForm.info_generale?.date_limite_reservation || ''
                      }
                      onChange={(e) => {
                        const value = e.target.value;
                        setVoyageForm((prev) => ({
...
export default VoyageFormComplete;
