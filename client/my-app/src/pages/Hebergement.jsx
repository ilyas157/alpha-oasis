...
import HotelFilterBar from '../components/destination/Filters/HotelFilterBar';
import MyCard from '../components/Mock/MyCard';
...
const Hebergement = () => {
  ...
  const data = useFilteredData('hebergement', "",page, sort, setLoading);
  ...
  return (
    <>
      ...
      <main className="pt-[140px] bg-[#f3f0ed80]">
        ...
        <h1 className="text-5xl text-center lg:pl-90 mb-12">hebergement</h1>
        {/*    
          ...
          <div className="flex flex-grow flex-col justify-center ">
            <div className="flex flex-grow  lg:self-auto self-center lg:justify-start justify-center gap-10 w-fit h-fit flex-wrap">
              {loading ? (
                ...
                data.hebergements.map((hebergement, index) => (
                  <MyCard
                    key={index}
                    id = {hebergement.id}
                    prix={hebergement.prix_basique}
                    img={img}
                    name={hebergement.name}
                    description={hebergement.description}
                    note={hebergement.note}
                    city={hebergement.city}
                    country={hebergement.country}
                    services={hebergement.services}
                  />
                ))
              ) : (
                <div className="lg:absolute lg:top-3/4 lg:left-1/3">
                  <p className="text-gray-500 text-3xl p-5">
                    Aucun hébergement n'est trouvé avec ces filtres.
                  </p>
                </div>
              )}
            </div>
            ...
            <Pagination
              currentPage={page}
              numOfPages={Math.ceil(data.total / 6)}
              changePage={(e) => {
