import React from 'react';

const Avis = ({ src, user, time, titre, avis, stars }) => {
  return (
    <div className="carousel-item  ">
      <div className=" bg-white w-[370px] sm:w-[630px] shadow-xl rounded-4xl border-4 border-yellow-300 bg-base-400 py-10 px-4">
        <div className="w-fit flex justify-around gap-20 ">
          <img
            className="w-32 h-32 rounded-full object-cover"
            src="/src/assets/content/profile.avif"
            alt="profile photo"
          />
          <div className="flex flex-col gap-7">
            <p>
              {user} <br />
              {time}
            </p>
            <p className="font-bold text-xl ">{titre}</p>
          </div>
        </div>
        <div className="flex mt-5">
          {Array.from({ length: stars }, (_, i) => (
            <svg
              key={i}
              width="32"
              height="32"
              viewBox="0 0 24 24"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              transform="rotate(360 0 0)"
            >
              <path
                d="M12.6722 2.04308C12.5459 1.78707 12.2851 1.625 11.9996 1.625C11.7142 1.625 11.4534 1.78707 11.3271 2.04308L8.67288 7.4211L2.73788 8.28351C2.45536 8.32456 2.22065 8.52244 2.13243 8.79395C2.04421 9.06546 2.11779 9.36351 2.32222 9.56278L6.61682 13.749L5.603 19.66C5.55475 19.9414 5.67041 20.2257 5.90137 20.3936C6.13233 20.5614 6.43853 20.5835 6.69122 20.4506L11.9996 17.6598L17.3081 20.4506C17.5608 20.5835 17.867 20.5614 18.0979 20.3936C18.3289 20.2257 18.4445 19.9414 18.3963 19.66L17.3825 13.749L21.6771 9.56278C21.8815 9.36351 21.9551 9.06546 21.8669 8.79395C21.7786 8.52244 21.5439 8.32456 21.2614 8.28351L15.3264 7.4211L12.6722 2.04308Z"
                fill="#fff800"
              />
            </svg>
          ))}
          {Array.from({ length: 5 - stars }, (_, i) => (
            <svg
              key={i}
              width="32"
              height="32"
              viewBox="0 0 24 25"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              transform="rotate(360 0 0)"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M11.9996 2.125C12.2851 2.125 12.5459 2.28707 12.6722 2.54308L15.3264 7.9211L21.2614 8.78351C21.5439 8.82456 21.7786 9.02244 21.8669 9.29395C21.9551 9.56546 21.8815 9.86351 21.6771 10.0628L17.3825 14.249L18.3963 20.16C18.4445 20.4414 18.3289 20.7257 18.0979 20.8936C17.867 21.0614 17.5608 21.0835 17.3081 20.9506L11.9996 18.1598L6.69122 20.9506C6.43853 21.0835 6.13233 21.0614 5.90137 20.8936C5.67041 20.7257 5.55475 20.4414 5.603 20.16L6.61682 14.249L2.32222 10.0628C2.11779 9.86351 2.04421 9.56546 2.13243 9.29395C2.22065 9.02244 2.45536 8.82456 2.73788 8.78351L8.67288 7.9211L11.3271 2.54308C11.4534 2.28707 11.7142 2.125 11.9996 2.125ZM11.9996 4.56966L9.84348 8.93853C9.73423 9.15989 9.52306 9.31331 9.27878 9.34881L4.45745 10.0494L7.94619 13.4501C8.12296 13.6224 8.20362 13.8706 8.16189 14.1139L7.33831 18.9158L11.6506 16.6487C11.8691 16.5338 12.1302 16.5338 12.3486 16.6487L16.661 18.9158L15.8374 14.1139C15.7957 13.8706 15.8763 13.6224 16.0531 13.4501L19.5418 10.0494L14.7205 9.34881C14.4762 9.31331 14.2651 9.15989 14.1558 8.93853L11.9996 4.56966Z"
                fill="#fff800"
              />
            </svg>
          ))}
        </div>
        <p className="mt-4 leading-relaxed text-md text-justify">{avis}</p>
      </div>
    </div>
  );
};

export default Avis;
