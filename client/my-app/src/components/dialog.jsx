  ...
  <DialogPortal>
    ...
    >
      ...
      <DialogPrimitive.Close
        className="absolute right-4 top-4 rounded-full p-1 hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-300 transition"
      >
...
const DialogFooter = ({ className, ...props }) => (
  <div
    className={`flex flex-col-reverse sm:flex-row sm:justify-between sm:space-x-2 ${className ? ` ${className}` : ""}`}
    {...props}
  />
);
...
const DialogTitle = React.forwardRef(({ className, ...props }, ref) => (
  <DialogPrimitive.Title
    ref={ref}
    className={`text-lg font-semibold text-gray-900 leading-none tracking-tight ${className ? ` ${className}` : ""}`}
    {...props}
  />
));
